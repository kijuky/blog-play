...
object SqlLogging {
  ...
  def install(maxSqlChars: Int = 240): Unit = {
    GlobalSettings.queryCompletionListener = (sql, _, _) => {
      val stack = bestFrame(new Exception().getStackTrace)
    ...
    GlobalSettings.queryFailureListener = (sql, _, cause) => {
      ...
      logger.error(formatLine("NG", sql, Option(cause.getMessage), stack, maxSqlChars))
    }
  }

  private def formatLine(status: String, sql: String, message: Option[String], stack: Option[String], maxSqlChars: Int): String = {
    val cleanedSql = oneLine(sql).take(maxSqlChars)
    val msg = message.filter(_.nonEmpty).map(m => s" msg=${oneLine(m).take(120)}").getOrElse("")
    val where = stack.map(s => s" at=$s").getOrElse("")
  ...
  private def bestFrame(stack: Array[StackTraceElement]): Option[String] = {
    ...
    val preferred = stack.find(e => e.getClassName.startsWith("controllers.") || e.getClassName.startsWith("services.") || e.getClassName.startsWith("loader."))
